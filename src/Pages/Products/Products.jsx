...
export default function Products() {
  ...
  async function addProductToCart(productId) {
    if (!UserToken) {
      await Swal.fire({
        icon: "warning",
        title: "Please login first",
        text: "You must be logged in to use Cart",
        confirmButtonText: "OK"
      });
      return;
    }
  ...
  async function removeProductFromWishlist(productId) {
    setHeartLoadingId(productId);
    try {
      return await deleteWishlistProduct(productId);
    } catch (error) {
      throw error;
    } finally {
      setHeartLoadingId(null);
  ...
  return (
    <div className='px-36 py-10'>
      ...
      <div className="flex flex-wrap px-4 pb-10">
        {currentProducts.map((product) => (
          <div key={product._id} className='w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-6'>
            <div className="w-full group relative bg-white shadow-md hover:shadow-xl rounded-md h-96 p-4 overflow-hidden product">
                ...
                <div className="mt-4 text-center">
                  ...
                  <div className='flex px-4 justify-center'>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-gray-500 ml-2 text-sm">({product.ratingsQuantity || 0} Reviews)</span>
                    </div>
                  </div>
                </div>
              </Link>
              ...
              <div className="absolute top-6 right-2 flex flex-col space-y-2">
                <button
                  ...
                  className="bg-white p-2 rounded-full shadow hover:bg-gray-300 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  disabled={heartLoadingId === product._id}
                >
                  {heartLoadingId === product._id ? (
                ...
                <Link to={`/product/${product.category._id}/${product._id}`} className="bg-white p-2 rounded-full shadow hover:bg-gray-300">
      ...
      <div className="flex justify-center mt-6 space-x-2 mb-20">
        {Array.from({ length: totalPages }, (_, i) => (
          <button
            key={i}
            onClick={() => setCurrentPage(i + 1)}
            className={`px-4 py-2 rounded cursor-pointer ${currentPage === i + 1 ? 'bg-black text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
          >
            {i + 1}
          </button>
        ))}
